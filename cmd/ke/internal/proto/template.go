...
enum Reason {
	...
	REASON_UNSPECIFIED          = 0 [(errors.code) = 500];
	// 请求错误，包含参数校验不过
	REASON_BAD_REQUEST          = 1 [(errors.code) = 400];
	// 找不到资源
	REASON_NOT_FOUND            = 2  [(errors.code) = 404];
	// 服务内部未知
	REASON_INTERNAL_ERROR       = 3 [(errors.code) = 500];
	// 数据库异常
	REASON_DB_UNAVAILABLE       = 4 [(errors.code) = 503];
	// 缓存异常
	REASON_CACHE_UNAVAILABLE    = 5 [(errors.code) = 503];
	// mq异常
	REASON_MQ_UNAVAILABLE       = 6 [(errors.code) = 503];
}
